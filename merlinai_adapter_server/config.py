...
ADAPTER_API_KEY = os.getenv("ADAPTER_API_KEY", "sk-123")
LOG_LEVEL_NAME = os.getenv("LOG_LEVEL", "INFO").upper()
LOG_TO_FILE = os.getenv("LOG_TO_FILE", "true").lower() in {"1", "true", "yes", "on"}
PROJECT_DIR = Path(__file__).resolve().parent.parent
LOG_FILE_PATH = PROJECT_DIR / "logs" / "adapter.log"
LOG_MAX_BYTES = 1_048_576
LOG_BACKUP_COUNT = 3
TOKEN_REFRESH_BUFFER_SECONDS = 60
AUTH_REQUEST_TIMEOUT_SECONDS = max(float(os.getenv("AUTH_REQUEST_TIMEOUT_SECONDS", "20")), 1.0)
MERLIN_REQUEST_TIMEOUT_SECONDS = max(float(os.getenv("MERLIN_REQUEST_TIMEOUT_SECONDS", "45")), 1.0)
...
TOOL_SYSTEM_MAX_CHARS = max(int(os.getenv("TOOL_SYSTEM_MAX_CHARS", str(max(TOOL_MESSAGE_MAX_CHARS, 12000)))), 0)
TOOL_TOOL_RESULT_MAX_CHARS = max(
    int(os.getenv("TOOL_TOOL_RESULT_MAX_CHARS", str(max(TOOL_MESSAGE_MAX_CHARS, 6000)))),
    0,
)
TOOL_TOOL_ARGUMENTS_MAX_CHARS = max(
    int(os.getenv("TOOL_TOOL_ARGUMENTS_MAX_CHARS", str(max(TOOL_MESSAGE_MAX_CHARS, 4000)))),
    0,
)
TOOL_PARAMETER_DESCRIPTION_MAX_CHARS = max(
    int(os.getenv("TOOL_PARAMETER_DESCRIPTION_MAX_CHARS", str(max(TOOL_DESCRIPTION_MAX_CHARS, 300)))),
    0,
)
