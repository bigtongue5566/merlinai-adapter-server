...
5. 若回來內容可解析為 `{"type":"tool_calls"...}`，proxy 會轉成 OpenAI `message.tool_calls`
6. 若 `tool_choice` 是 `required` 或指定函式，但仍解析不到工具呼叫，proxy 直接回 `422`
7. 若上游回傳 malformed payload，proxy 會用 `repair` prompt 重試一次
8. 若在 `tool_choice=auto` 的多步工具流程中，模型提前回成一般 `message`，proxy 可能再做一次 `agentic_repair`
...
如果你要把 `proxy.log` 整理成 Markdown 報表，可執行：

```bash
uv run python scripts/build_log_report.py --log logs/proxy.log --out logs/report.md
```
